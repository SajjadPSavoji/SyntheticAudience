...
\item We show that a role-played synthetic audience is a usable proxy for a real crowd. It reproduces the differences in taste between viewer groups at a correlation $9\times$ that of a persona-free VLM.

\item We show that adding the synthetic audience as the critic in an auto-research style editing loop draws $3\times$ reacher guidance than a generic critic and measurably improves aesthetic scores.
\end{itemize}
...
\paragraph{Vision-language models as aesthetic judges.} CLIP-based scorers made zero-shot aesthetic prediction practical \citep{schuhmann2022, ke2023vila}. Instruction-tuned multimodal LLMs changed the interface. When quality is mapped onto a small set of text-defined levels, the predicted scores correlate well with human ratings \citep{wu2024qbench, wu2024qalign}. The remaining gap is in the reasoning behind the score. Evaluations on specific aesthetics benchmarks show that general-purpose MLLMs are still below human annotators on fine-grained aesthetic perception and interpretation \citep{huang2024aesbench, zhou2024uniaa}, which is the motivation for expert models fine-tuned on a specific domain\citep{huang2024aesexpert}. One consensus judgment per image is what these methods have in common. They do not model how that judgment varies across viewers.

\paragraph{Simulating Human Behavior with language models.} Conditioning a language model on demographic characteristics reproduces survey response distributions with surprising fidelity \citep{argyle2023}. Still, the follow-up literature is more cautious: synthetic responses show systematically lower variance than human data \citep{bisbee2024synthetic}, opinion distributions are misaligned with several demographic groups \citep{santurkar2023whose}, and persona prompts can tip into stereotypes \citep{cheng2023compost}. Two lessons carry over. Aggregate-level simulation is more trustworthy than individual-level simulation, and calibration is a prerequisite rather than a refinement. This work is almost entirely text; whether persona conditioning transfers to visual aesthetic judgment is open, and that is the question we answer.

\paragraph{Feedback loops for generation and editing.} Reward-driven refinement is standard, whether through a learned preference model \citep{xu2023imagereward, kirstain2023pickapic}, policy-gradient RL \citep{fan2023dpok, black2024ddpo}, differentiable reward backpropagation \citep{clark2024draft, prabhudesai2023alignprop}, or direct preference optimization \citep{wallace2024dpo}. In parallel, language-feedback self-refinement established the critique-then-revise pattern \citep{madaan2023selfrefine, shinn2023reflexion, gou2024critic}, and multimodal variants dispatch a VLM to find and fix faults \citep{wu2024sld, wang2024genartist}, with instruction-based editors as the actuator \citep{brooks2023instructpix2pix, flux2024}. Two gaps persist. Every reward here is a scalar consensus signal, so none represents an audience; and when one model both proposes and scores an edit, improvement can be self-fulfilling, a self-bias documented directly in LLM self-evaluation \citep{xu2024selfbias}. AutoPolish addresses both: the reward is a plural audience whose critique is natural language, and the proposer and the frozen held-out grader \citep{schuhmann2022, oquab2023} are different model families, with the grader never shown the instruction.
...
Photo aesthetics ranking network with attributes and content adaptation. In {\it ECCV}.

\bibitem{chang2017pccd}
Chang, K.-Y., Lu, K.-H.\ and Chen, C.-S.\ (2017)
Aesthetic critiques generation for photos. In {\it ICCV}.

\bibitem{kang2020eva}
Kang, C., Valenzise, G.\ and Dufaux, F.\ (2020)
EVA: An explainable visual aesthetics dataset.
In {\it Joint Workshop on Aesthetic and Technical Quality Assessment of Multimedia and Media Analytics for Societal Trends (ACM MM)}.

\bibitem{talebi2018nima}
Talebi, H.\ and Milanfar, P.\ (2018)
NIMA: Neural image assessment. {\it IEEE Transactions on Image Processing} {\bf 27}(8):3998-4011.

\bibitem{jin2018cjs}
Jin, X.\ et al.\ (2018)
Predicting aesthetic score distribution through cumulative Jensen-Shannon divergence. In {\it AAAI}.

\bibitem{ren2017personalized}
Ren, J., Shen, X., Lin, Z., Mech, R.\ and Foran, D.J.\ (2017)
Personalized image aesthetics. In {\it ICCV}.
...
Wu, H., Zhang, Z., Zhang, E., Chen, C., Liao, L.\ et al.\ (2024)
Q-Bench: A benchmark for general-purpose foundation models on low-level vision. In {\it ICLR}.
...
Zhou, Z.\ et al.\ (2024)
UNIAA: A unified multi-modal image aesthetic assessment baseline and benchmark. Preprint.

\bibitem{huang2024aesexpert}
Huang, Y.\ et al.\ (2024)
AesExpert: Towards multi-modality foundation model for image aesthetics perception.
In {\it ACM Multimedia}.
...
Whose opinions do language models reflect? In {\it ICML}.

\bibitem{cheng2023compost}
Cheng, M., Piccardi, T.\ and Yang, D.\ (2023)
CoMPosT: Characterizing and evaluating caricature in LLM simulations. In {\it EMNLP}.

\bibitem{fan2023dpok}
Fan, Y.\ et al.\ (2023)
DPOK: Reinforcement learning for fine-tuning text-to-image diffusion models. In {\it NeurIPS}.

\bibitem{clark2024draft}
Clark, K., Vicol, P., Swersky, K.\ and Fleet, D.J.\ (2024)
Directly fine-tuning diffusion models on differentiable rewards. In {\it ICLR}.

\bibitem{prabhudesai2023alignprop}
Prabhudesai, M., Goyal, A., Pathak, D.\ and Fragkiadaki, K.\ (2023)
Aligning text-to-image diffusion models with reward backpropagation. Preprint.

\bibitem{madaan2023selfrefine}
Madaan, A.\ et al.\ (2023)
Self-Refine: Iterative refinement with self-feedback. In {\it NeurIPS}.

\bibitem{shinn2023reflexion}
Shinn, N., Cassano, F., Berman, E., Gopinath, A., Narasimhan, K.\ and Yao, S.\ (2023)
Reflexion: Language agents with verbal reinforcement learning. In {\it NeurIPS}.

\bibitem{gou2024critic}
Gou, Z.\ et al.\ (2024)
CRITIC: Large language models can self-correct with tool-interactive critiquing. In {\it ICLR}.

\bibitem{wu2024sld}
Wu, T.-H., Lian, L., Gonzalez, J.E., Li, B.\ and Darrell, T.\ (2024)
Self-correcting LLM-controlled diffusion models. In {\it CVPR}.

\bibitem{wang2024genartist}
Wang, Z.\ et al.\ (2024)
GenArtist: Multimodal LLM as an agent for unified image generation and editing. In {\it NeurIPS}.

\bibitem{brooks2023instructpix2pix}
Brooks, T., Holynski, A.\ and Efros, A.A.\ (2023)
InstructPix2Pix: Learning to follow image editing instructions. In {\it CVPR}.

\bibitem{xu2024selfbias}
Xu, W., Zhu, G., Zhao, X., Pan, L., Li, L.\ and Wang, W.Y.\ (2024)
Pride and prejudice: LLM amplifies self-bias in self-refinement. In {\it ACL}.
