...
\paragraph{Vision-language models as aesthetic judges.} CLIP-based scorers made zero-shot aesthetic prediction practical \citep{schuhmann2022, ke2023vila}, and instruction-tuned multimodal LLMs made it conversational: mapping quality onto discrete text-defined levels gives strong score prediction \citep{wu2024qbench, wu2024qalign}, while dedicated benchmarks report that general-purpose models still trail humans on fine-grained aesthetic reasoning \citep{huang2024aesbench, zhou2024uniaa, huang2024aesexpert}. All of it targets a single consensus judgment. None simulates a distribution of viewers.

\paragraph{Simulating people with language models.} Conditioning a language model on a demographic backstory reproduces survey response distributions with surprising fidelity \citep{argyle2023}, but the follow-up literature is more cautious: synthetic responses show systematically lower variance than human data \citep{bisbee2024synthetic}, opinion distributions are misaligned with several demographic groups \citep{santurkar2023whose}, and persona prompts can tip into caricature \citep{cheng2023compost}. Two lessons carry over. Aggregate-level simulation is more trustworthy than individual-level simulation, and calibration is a prerequisite rather than a refinement. This work is almost entirely text; whether persona conditioning transfers to \emph{visual} aesthetic judgment is open, and that is the question we answer.
