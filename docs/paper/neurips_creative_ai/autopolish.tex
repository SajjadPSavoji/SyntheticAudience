...
\paragraph{Feedback loops for generation and editing.} Reward-driven refinement is the standard, whether through a learned preference model \citep{xu2023imagereward, kirstain2023pickapic}, policy-gradient RL \citep{fan2023dpok, black2024ddpo}, differentiable reward backpropagation \citep{clark2024draft, prabhudesai2023alignprop}, or direct preference optimization \citep{wallace2024dpo}. A parallel line of work replaces the scalar with language \citep{madaan2023selfrefine, shinn2023reflexion, gou2024critic} and established the critique-then-revise loop, in which a model criticizes its own output in natural language and then rewrites it. Multimodal variants hand this role to a VLM that locates and repairs the fault \citep{wu2024sld, wang2024genartist}, with instruction-based editors as the actuator \citep{brooks2023instructpix2pix, flux2024}. Two gaps persist. First, every reward here is a scalar: disagreement between viewers is aggregated into a single number before the model sees it, so the signal represents a consensus rather than an audience. Second, when one model both proposes an edit and scores it, the measured improvement is circular, a self-bias documented for LLM self-evaluation \citep{xu2024selfbias}. AutoPolish addresses both. The reward comes from a group of audience. It takes the form of natural-language critique rather than a number, and the proposer and the frozen held-out evaluator \citep{schuhmann2022, oquab2023} come from different model families.

\section{Method}

\paragraph{Persona cards.} A persona card is a short textual description of one real annotator, generated deterministically from the attributes their dataset provides: age band, education, art and photography familiarity, Big-Five scores, and nationality. Attributes that a dataset does not provide are left out, not filled in with estimated values. A typical card reads \emph{``Viewer profile: 35 to 44, master's degree, high art familiarity, moderate photography experience, nationality: Belgium. Personality (Big-5, 1 to 5): O 4.2, C 3.1, E 2.8, A 3.9, N 2.5.''} We use the same template for the annotators we sample panels from and for the held-out annotators we evaluate on. Our claims are therefore limited to the attributes that the card actually contains.
