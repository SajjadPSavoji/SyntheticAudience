...
We then put the audience to work. AutoPolish is an autonomous editing loop in which the synthetic audience is the critic: it inspects the current image, aggregates its complaints into one short imperative instruction, and an image editor applies it. An independent held-out judge, from a different model family and blind to the instruction, decides whether the edit truly improved the image, so the loop cannot certify its own success. Our contributions are:
...
\begin{figure}[!htb]
  ...
  \caption{\textbf{The loop over time.} The committed best-so-far image after refinement steps $0, 2, 4, 6$ and $10$ for the \textsc{society} critic, with the held-out aesthetic score above each cell. The audience's complaints accumulate across steps, so the edits arrive as visible increments rather than one jump, and the score rises monotonically by construction. The full seven-image version is Figure~\ref{fig:c4-prog}.}
  \label{fig:c4prog}
\end{figure}
