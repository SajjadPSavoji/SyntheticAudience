"""Pixel pipeline: judge the *unlabeled* images-9k corpus with the PARA model.

/shared/bofeng/code/aesthetic-predictor/dataset/images-9k-resized/ is a flat
folder of ~8.5k resized photos with no annotations — no raters, no ground-truth
scores. So this script is deliberately the persona-blind arm of
...
This is the exact same design as script/webscrap_pipeline.py, only pointed at a
different image folder — see that file for the long-form rationale. What carries
over from para_pipeline.py, unchanged: the ScoreDimension grids and their prompt
glosses, the generic system prompt, the question builder, and the response
parser. Metrics are imported where they still have a referent and recomputed
where they don't:
...
Run from the repo root (persona conda env):
    ...
    python script/pixel_pipeline.py --samples-per-image 8 --temperature 0.7
    python script/pixel_pipeline.py --n-images 5 --dimensions aestheticScore
    python script/pixel_pipeline.py --analyze-only data/logs/pixel_<ts>.json
"""
...
# this file is run from anywhere, not just via ``python script/pixel_pipeline.py``.
...
# The corpus lives outside this repo; override with --image-dir if it moves.
PIXEL_IMAGE_DIR = Path(
    "/shared/bofeng/code/aesthetic-predictor/dataset/images-9k-resized"
)
...
def print_summary(metrics: dict) -> None:
    print("\n" + "=" * 72)
    print("Pixel persona-blind summary (no ground truth — corpus-relative only)")
    print("=" * 72)
    print(
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Judge the unlabeled images-9k corpus with the persona-blind PARA "
            "prompts, sampling each image several times to get a score distribution."
        )
    )
    parser.add_argument(
        "--image-dir",
        type=Path,
        default=PIXEL_IMAGE_DIR,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Where to write the JSON log. Defaults to data/logs/pixel_<timestamp>.json.",
    )
    parser.add_argument(
        "--checkpoint-interval",
        type=float,
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.output:
        ...
        output_path = DEFAULT_LOG_DIR / f"pixel_{ts}.json"
